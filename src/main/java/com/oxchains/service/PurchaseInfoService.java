...
public class PurchaseInfoService extends BaseService  {
    ...
    public RespDTO<List<PurchaseInfo>> queryPurchaseInfoByUniqueCode(String UniqueCode,String Token){
        String jsonStr = chaincodeService.getPayloadAndTxid("searchByView", new String[]{"{\"designDocName\":\"myview\",\"viewName\":\"view-purchase\",\"key\":\""+UniqueCode+"\"}"});
        ...
        if (purchaseInfoDTO.getList().isEmpty()) {
            ...
            if(goodsDTO!=null && goodsDTO.getList().size()>0){
                ...
                if(!StringUtils.isEmpty(parentCode)){
                    jsonStr = chaincodeService.getPayloadAndTxid("searchByView", new String[]{"{\"designDocName\":\"myview\",\"viewName\":\"view-purchase\",\"key\":\""+parentCode+"\"}"});
                    if (StringUtils.isEmpty(jsonStr)) {
                        return RespDTO.fail("没有数据");
                    }
                    ...
                    if (purchaseInfoDTO.getList().isEmpty()) {
                        ...
                        if(goodsDTO!=null && goodsDTO.getList().size()>0){
                            ...
                            if(!StringUtils.isEmpty(pparentCode)){
                                jsonStr = chaincodeService.getPayloadAndTxid("searchByView", new String[]{"{\"designDocName\":\"myview\",\"viewName\":\"view-purchase\",\"key\":\""+pparentCode+"\"}"});
                                if (StringUtils.isEmpty(jsonStr)) {
                                    return RespDTO.fail("没有数据");
                                }
